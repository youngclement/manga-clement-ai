...
import { Sparkles, Zap, X, Wand2, BookOpen, Target } from 'lucide-react';
...
export default function PromptPanel({
    ...
    const handleChapterSelect = (targetPages: number) => {
    ...
    const getRemainingPages = (target: number) => {
        return Math.max(0, target - currentPageCount);
    };
            ...
            <div className="flex-1 flex flex-col gap-4">
                ...
                <textarea
                    ...
                    placeholder={isAutoContinue
                        ? "Story direction suggestions (optional)... E.g.: 'The hero discovers a secret', 'A battle begins', 'They meet a new character'..."
                        : hasPages
                    ...
                    <div className="space-y-2">
                        ...
                        {batchProgress.current >= 1 && batchProgress.current < batchProgress.total && (
                            <div className="text-[9px] text-zinc-400 flex items-center gap-1 leading-relaxed">
                                ...
                                <span>
                                    <strong>2-Step Process:</strong> ① AI creates new prompt from page {batchProgress.current}
                                    → ② Generate image from that prompt
                                </span>
                            </div>
                        )}
                    </div>
                ...
                <div className="grid grid-cols-3 gap-2 sm:gap-3">
                    <button
                        ...
                        className="px-3 sm:px-4 py-3 sm:py-3.5 bg-gradient-to-b from-amber-400 to-amber-600 hover:from-amber-500 hover:to-amber-700 disabled:from-zinc-800 disabled:to-zinc-900 disabled:text-zinc-600 text-black font-manga text-xs sm:text-sm rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-[0_4px_0_0_rgb(180,83,9)] hover:shadow-[0_4px_0_0_rgb(180,83,9)] hover:scale-[1.02] active:shadow-[0_1px_0_0_rgb(180,83,9)] disabled:shadow-none active:translate-y-1 disabled:translate-y-0 disabled:cursor-not-allowed ring-2 ring-transparent hover:ring-amber-500/30 touch-manipulation min-h-[48px] sm:min-h-[52px]"
                    >
                    ...
                    {batchLoading ? (
                        ...
                        >
                            ...
                            <span>CANCEL ({batchProgress?.current || 0}/{batchProgress?.total || 0})</span>
                        </button>
                    ) : (
                        <>
                            <Popover open={batchPopoverOpen} onOpenChange={setBatchPopoverOpen}>
                                ...
                                <PopoverContent className="w-48 bg-zinc-950/95 border-zinc-800/60 backdrop-blur-md p-2 shadow-xl" align="center">
                                    <div className="space-y-1">
                                        <div className="px-3 py-1.5 text-[10px] text-zinc-500 uppercase tracking-wider font-semibold" style={{ fontFamily: 'var(--font-inter)' }}>
